...
(À la suite de ma dernière phrases (Mickael))
Une fois le workspace choisi, une vue sur l'arborescence des projets est disponible.
A partir de cet arborescence l'utilisateur peut enfin interagir avec.
Plusieurs intéractions sont disponibles avec les éléments de la vue.
Le double clic entraîne l'ouverture du fichier ou l'ouverture/fermeture d'un dossier/projet.
Le clic droit quand à lui ouvre un menu déroulant, dans lequel on retrouve les intéractions communes : déplacement, copie, renommage, ouverture, suppression, ajout de fichier, de dossier etc...

Nous allons maintenant détailler comment ces intéractions sont gérées.
Premièrement, lors de la sélection du workspace, on instancie un controleur de la classe WorkSpaceTreeController faisant la liaison entre le modèle et la vue. Ce controleur instancie également un objet de la classe WorkSpace. Lorsqu'une intéraction a lieu avec un élément, un index est donné à ce controleur, ainsi que la méthode à appliquer au modèle. L'index est un objet de classe QModelIndex donné par les classes de Qt c'est à partir de lui qu'on va récupérer l'élément visé dans le modèle. Le controleur s'occupe de transformer l'index en liste. Cette liste est donc le « chemin » pour trouver l'élément.
Cette liste est donnée au workspace.
Et récursivement l'algorithme suivant est appliquée :
L'élément qui reçoit la liste, vérifie que la liste est de taille supérieure à 1.
Si la taille est supérieure à 1 : l'élément retire le premier index de cette liste, et la donne à l'élément correspondant à l'index.
Si la taille est égale à 1 : l'élément retire le dernier index et ordonne à l'élément correspondant à cet index d'éffectuer telle action.

Chargement de l'image intéractions

(Met ça à la suite je ferai une transition!)

En supposant le fait, que des utilisateurs viendrait à utiliser notre application pour gérer un nombre important de sites web, nous avons étudié la réaction de notre logiciel face à un nombre important de fichiers.
Pour cela, nous avons importé environ 16000 fichiers dans un workspace.
Testé l'ouverture de celui-ci. Pour une telle base, le temps pour l'ouverture était de l'ordre de 6 à 7 secondes, sur un PC équipé d'un processeur i3.
Mais le plus inquiétant n'était pas vraiment cette attente.
En effet en étudiant l'occupation mémoire de l'application, nous nous sommes aperçus grâce à la commande top, qu'a chaque réouverture du même workspace, l'occupation en mémoire augmentait de 2 à 3% sur une mémoire d'un giga.
Nous avons donc utilisé un outil conçu pour corriger ces fuites : Valgrind.
L'analyse de notre logiciel avec cet outil nous a permis de trouver plusieurs mauvaises utilisations d'allocations et libérations mémoires.
En effet nous nous sommes aperçus que des allocations mémoires se faisaient sur  un même objet dans une boucle, sans jamais libérer ce dernier.
Valgrind nous a fait remarqué que sur certains objets, ou pointeurs, des confusions entre les libérateurs free, delete et delete[] étaient présentes, conduisant ainsi à une mauvaise libération de la mémoire.
Finalement les fuites se sont fortement réduites passant à 0.1%. Malheureusement, certaines ne sont pas inhérente à notre code, Valgrind notant des fuites dans des appels de Qt. Nous ne sommes pas parvenus à atteindre le 0%.
En ce qui concerne le temps de chargement, nous nous sommes aperçus qu'à chaque création d'un élément (fichier, dossier, projet etc...) une icône était créée pour ce dernier.
Nous avons donc choisi d'insérer dans la classe Tools qui contient un certain nombre d'éléments statiques, de charger toutes les icônes du projet une fois statiquement, et de lier ces icônes aux éléments.
Finalement le temps d'ouverture de ce workspace est tombé à environ une seconde. Ce qui est nettement mieux.

				Dans ce cas de figure, le scan du projet ajouté est lancé, et celui-ci va alors ajouter tous les fichiers et dossiers qu'il contient à ses vecteurs respectifs \emph{file} et \emph{folders}.\\
				Le déclenchement de ces parcours se réalise à l'instant où l'utilisateur sélectionne l'espace de travail de son choix via le menu de l'application.
				...
				\begin{subsection}{Coloration}
					...
					\begin{figure}[h]
						\begin{center}
							...
							\caption{La coloration en action en pratique}
						\end{center}
					\end{figure}~\\					
				\end{subsection}
